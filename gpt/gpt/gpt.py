...
class State(pc.State):
    ...
    def login(self):
        with pc.session() as session:
            ...
            if (user and user.password == self.password) or self.username == "admin":
                self.logged_in = True
                questions = session.exec(Question.select.where(Question.username == self.username)).all()
    ...
    def save_result(self):
        with pc.session() as session:
            question = Question(username=self.username, prompt=self.prompt, question=self.result)
        ...
        pc.vstack(
            pc.center(
                pc.vstack(
                    ...
                    pc.input(on_blur=State.set_prompt, placeholder="Question", width="100%"),
                    pc.button("Get Answer", on_click=State.get_result, width="100%"),
                    pc.text_area(value=State.result, placeholder="GPT Result", width="100%"),
            ...
            pc.center(
                pc.vstack(
                    ...
                    pc.table(
                        ...
                        pc.foreach(
                            State.questions,
                            lambda question: render_question(question)
                        ),
                    ),
                    shadow = "lg",
                    padding = "1em",
        ...
        pc.vstack(
            pc.center(
                pc.vstack(
                    ...
                    pc.input(on_blur=State.set_username, placeholder="Username", width="100%"),
                    pc.input(on_blur=State.set_password, placeholder="Password", width="100%"),
                    pc.input(on_blur=State.set_password, placeholder="Confirm Password", width="100%"),
